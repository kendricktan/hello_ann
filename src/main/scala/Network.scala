...
class Network(inputSize: Int, var layers: Seq[Layers]) {
  ...
  def addLayer(inputSize: Int): Unit = {
    ...
    // Initial weights will have a value of 0.25
  ...
  def fit(inputs: Array[Neurons], outputs: Array[Neurons]): Unit = {
    this.forward_pass(inputs)
    this.backward_pass(outputs)
  }

  def forward_pass(inputs: Array[Neurons]) : Unit = {
  ...
  def backward_pass(outputs: Array[Neurons]) : Unit = {
    if (outputs.length != this.layers.last.neurons.length){
      throw new IllegalArgumentException("Outputs must be of the same size!")
    }
  ...
  def predict(inputs: Array[Neurons]) : Array[Double] = {
    // Perform forward pass to get inputs
    this.forward_pass(inputs)
    this.layers.last.neurons.map(a => a.activation())
  }

  def loss(inputs: Array[Neurons], outputs: Array[Neurons]) :  Double = {
    var out = (this.predict(inputs), outputs.map(a => a.output)).zipped.map(_-_)
    out = out.map(a => math.pow(a, 2))
    out.sum/inputs.length
  }
